...
enum RouterChineseText {
  static let values: [String: String] = [
    ...
    "The reset for %@ may have used a credit. Retry checks the saved attempt; if limits recovered, inspect account usage.": "%@ 的重置可能已消耗额度。重试会检查已保存的请求；如果用量限制已恢复，请查看账户用量。",
    "The reset status for %@ could not be verified.": "无法确认 %@ 的重置状态。",
    "The reset status for %@ could not be verified: %@": "无法确认 %@ 的重置状态：%@",
    "Reset credit status unknown": "重置额度状态未知",
    "Retry saved attempt": "重试已保存的请求",
    "Waiting for a fresh reset credit balance": "等待更新重置额度余额",
    "Reset credit balance is being refreshed": "正在更新重置额度余额",
    "A banked reset credit was already used on %@; no additional credit was spent.": "已在 %@ 上使用过一个已存重置额度；没有额外消耗额度。",
    "Sign in to use a reset credit.": "登录后才能使用重置额度。",
    "A core rate limit must be at least 90% spent.": "核心用量限制必须已使用至少 90%。",
    "Use this banked reset?": "使用这个已存重置额度？",
    "A reset is available near a rate limit (at least 90% used). It immediately resets this account's usage limits, spends one banked reset credit, and cannot be undone.": "当用量接近上限（已使用至少 90%）时可重置。操作会立即重置此账户的用量限制，消耗一个已存重置额度，且无法撤销。",
    "%d available": "可用 %d 个",
    "Use reset credit": "使用重置额度",
    "Reset credit used": "已使用重置额度",
    "Reset credit was not used": "未使用重置额度",
    "RESET": "重置额度",
    "banked reset credits available": "可用的已存重置额度",
    "%d banked reset credits available for %@": "有 %d 个已存重置额度可用于 %@",
    "Use a banked reset credit on %@": "在 %@ 上使用已存重置额度",
    "Opens a confirmation; usable when a rate limit is at least 90% spent": "打开确认对话框；用量达到上限的 90% 时可使用",
    "Used one banked reset credit on %@.": "已在 %@ 上使用一个已存重置额度。",
    "%@ has no banked reset credit available.": "%@ 没有可用的已存重置额度。",
    "%@ is not near its rate limit, so no credit was used.": "%@ 尚未接近用量上限，因此未使用额度。",
    "A banked reset was already used on %@.": "已在 %@ 上使用过已存重置额度。",
    ...
    "Click to prefer this subscription": "点击将此订阅设为首选",
    "Double-click to prefer this subscription": "双击将此订阅设为首选",
    "Next turn uses this subscription": "下一轮将使用此订阅",
    "In rotation, position %d": "轮换中的第 %d 位",
    "Usage probe failed for this subscription": "此订阅的用量探测失败",
    "Out of rotation: quota is spent": "已退出轮换：额度已用尽",
    "Out of rotation": "已退出轮换",
    "%@ limit resets in": "%@ 限制将在以下时间后重置",
    "Limit resets in": "限制将在以下时间后重置",
    "rotation position %d": "轮换位置 %d",
    "out of rotation": "已退出轮换",
    ...
    "Engineering router": "工程路由",
    "Unavailable from this Router version": "此路由版本不支持",
    "Preset %@ · policy needs repair": "预设 %@ · 策略需要修复",
    "Preset %@ · status unavailable": "预设 %@ · 状态不可用",
    "Preset %@ · %@": "预设 %@ · %@",
    "effort %@": "思考强度 %@",
    "healthy": "健康",
    "needs attention": "需要处理",
    "Engineering routing status is unavailable or needs repair.": "工程路由状态不可用或需要修复。",
    "Engineering routing enabled for new engineering runs.": "已为新的工程任务启用工程路由。",
    "Engineering routing disabled for new engineering runs.": "已为新的工程任务禁用工程路由。",
