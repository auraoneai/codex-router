...
// the reserve the user already pays for -- and it is never a picker event,
...
// user selected. A bare slug ("gpt-5.6-sol") is native ChatGPT traffic that
// Codex hardwired. Same rule as `isNativeOpenAIRoute`.
...
function readSettings() {
  ...
  try {
    ...
    return {
      ...
      // Threads whose own picker the user moved after synchronization was
...
function readObservedThreadRows() {
  ...
  try {
    ...
    // The app owns this database and may be mid-write or mid-migration. An
    // unreadable one means "observe nothing", never "every thread changed".
    return [];
  } finally {
    database?.close();
  }
...
// Every mutation routes through here, and a thread the user pinned has to
...
function writeSettings(settings) {
  ...
  writePrivateJson(MODEL_SYNC_PATH, document, { directoryMode: 0o700 });
}
...
export function modelSyncSnapshot() {
  ...
  return {
    ...
    // A count, not the map: the user wants to know that some windows are
...
export function setModelSyncEnabled(enabled) {
  ...
  writeSettings({
    ...
    // Turning synchronization off abandons the pins with it: they exist only to
    // exempt threads from defaults that are no longer being applied.
    ...(enabled ? {} : { pinnedThreads: {} }),
  });
...
export function setModelSyncDefaults({ chatModel, cronModel } = {}) {
  ...
  const next = {
    ...
    observedModels: settings.observedModels,
  };
...
export function setModelSyncEfforts({ chatEffort, cronEffort } = {}) {
  ...
  for (const key of ["chatModel", "cronModel", "chatEffort", "cronEffort"]) {
...
// Carry the defaults and efforts forward across a pin/observation rewrite. Every
// mutation below has to preserve them, and spelling that out at each call site
// is how one of them eventually drops a field the user set.
function preservedSettings(settings) {
  return {
    enabled: true,
    selectedModel: settings.selectedModel,
    ...(settings.chatModel ? { chatModel: settings.chatModel } : {}),
...
export function refreshModelSyncFromCodex() {
  ...
  // redefining the global default. The user sets the defaults in one place --
  ...
  for (const row of changed) {
    ...
    // so it neither creates a pin nor clears one the user made.
  ...
  writeSettings({ ...preservedSettings(settings), pinnedThreads, observedModels });
  return modelSyncSnapshot();
}
...
export function synchronizedPayload(payload, { bypass = false, threadId } = {}) {
  ...
  // A user can override the automation effort without overriding its model,
  ...
  // own choice. That is true whether the user just moved the dropdown or moved
  // it several turns ago, so one rule covers both: the thread is pinned while it
  // disagrees with the default, and released the moment it agrees again.
  // Nothing here rewrites the defaults -- those are set in one place.
  ...
  // the user deliberately moved would be the same override they just rejected,
